...
public class CommonUserStatsCollector implements Collector<User, CommonUserStatsCollector.Accumulator, CommonStatsResult> {
    ...
    @Override
    public Function<Accumulator, CommonStatsResult> finisher() {
        return acc -> {
            double avg = acc.userCount > 0 ? (double) acc.totalDecks / acc.userCount : 0.0;
            return new CommonStatsResult(acc.totalDecks, avg, acc.activeUsers, acc.inactiveUsers, 0);
        };
    }
